...
import org.dhbw.mosbach.ai.cmd.extension.listener.result.ResultAccumulator;
...
@ThreadSafe
public class FailsafeTestPrintListener extends RunListener {

    private static final int INITIAL_TEST_NUMBER = 1;
    ...
    private ResultAccumulator accumulator = new ResultAccumulator();
    private RunTime totalRunTime = new RunTime();
    ...
    public void testRunStarted(Description description) throws Exception {
        ...
        this.testCount.set(countTests(testClass));

        long startTime = System.currentTimeMillis();
        totalRunTime.setStartTime(startTime);
        suiteRunTime.setStartTime(startTime);
    }
    ...
    public void testRunFinished(Result result) throws Exception {
        totalRunTime.setEndTime(System.currentTimeMillis());
        testPrinter.printTestEndResults(new TestSuiteResult(result, totalRunTime), testClassName);
    }
    ...
    public void testStarted(Description description) throws Exception {
        Class<?> currentTestClass = scanner.extractFirstTestClass(description);
        if (!currentTestClass.getName().equals(testClassName)) {
            this.testNumber.set(INITIAL_TEST_NUMBER);
            this.testClassName = currentTestClass.getName();
            this.testCount.set(countTests(currentTestClass));

            testPrinter.printTestSuiteStarted(description);
            testPrinter.printTestAnnotations(currentTestClass.getAnnotations());
    ...
    public void testFinished(Description description) throws Exception {
        ...
        accumulator.incrementRunCount();

        testPrinter.printTestFinished(testNumber.get(), description, testRunTime);

        if (testNumber.get() == testCount.get()) {
            suiteRunTime.setEndTime(System.currentTimeMillis());

            testPrinter.printTestEndResults(accumulator.buildResult(suiteRunTime), testClassName);
            accumulator.reset();
        }
    ...
    public void testFailure(Failure failure) throws Exception {
        ...
        accumulator.incrementFailureCount();
        accumulator.addFailure(failure);
    }
    ...
    public void testIgnored(Description description) throws Exception {
        ...
        accumulator.incrementIgnoreCount();
    }
}
